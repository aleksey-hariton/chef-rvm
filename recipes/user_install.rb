  ...
  ruby_block "Conditionally add RVM gpg key" do # ~FC022 this will be fixed in the LWRP rewrite
    block do
      ...
      if cmd.exitstatus == 0
        ...
        exec.command "#{gpg_command} --keyserver hkp://keys.gnupg.net --recv-keys #{node['rvm']['gpg']['key']}"
        exec.user user['user']
        exec.environment 'HOME' => user['home']
        exec.guard_interpreter :bash
        exec.not_if "#{gpg_command} -k #{node['rvm']['gpg']['key']} > /dev/null", user: user['user'], environment: { 'HOME' => user['home'] }
        exec.run_action :run
      else
